...
-- Drop existing constraint (auto-named in initial migration), then add a permissive one
...
ALTER TABLE protection_jobs
    ...
    CHECK (storage_mode IN ('filesystem', 'ipfs', 'both', 'archive', 'full'));
...
-- Tighten constraint to only the new allowed values
ALTER TABLE protection_jobs DROP CONSTRAINT protection_jobs_storage_mode_check;
ALTER TABLE protection_jobs
    ADD CONSTRAINT protection_jobs_storage_mode_check
