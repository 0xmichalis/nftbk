...
.PHONY: run-cli
run-cli:
	@SQLX_OFFLINE=true cargo run --bin nftbk-cli -- create $(filter-out $@,$(MAKECMDGOALS))
...
run-cli-server-test:
	@if [ -f config_ipfs.toml ]; then PIN_ARG="--pin-on-ipfs true"; else PIN_ARG=""; fi; \
		SQLX_OFFLINE=true cargo run --bin nftbk-cli -- server create --tokens-config-path config_tokens_test.toml --output-path nft_backup_test --server-mode true --force true $$PIN_ARG $(filter-out $@,$(MAKECMDGOALS))
...
.PHONY: run-server
run-server:
	@if [ -f config_auth.toml ]; then AUTH_ARG="--auth-config config_auth.toml"; else AUTH_ARG=""; fi; \
	    if [ -f config_ipfs.toml ]; then IPFS_ARG="--ipfs-config config_ipfs.toml"; else IPFS_ARG=""; fi; \
		cargo run --bin nftbk-server -- --unsafe-skip-checksum-check true --backup-parallelism 2 $$AUTH_ARG $$IPFS_ARG
