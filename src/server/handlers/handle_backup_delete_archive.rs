...
pub async fn handle_backup_delete_archive(
    ...
    handle_backup_delete_archive_core(&*state.db, &state.backup_task_sender, &task_id, requestor)
        .await
}
...
async fn handle_backup_delete_archive_core<DB: Database + ?Sized>(
    ...
    let (meta, problem) = verify_requestor_owns_task(
        ...
        task_id,
        requestor.clone(),
        &format!("/v1/backups/{task_id}/archive"),
    )
    .await;
    ...
    if meta.storage_mode == "ipfs" {
        let problem = ProblemJson::from_status(
            ...
            Some(format!("/v1/backups/{task_id}/archive")),
        );
    ...
    // Enqueue deletion task with ArchiveOnly scope and return 202
    ...
    if let Err(e) = backup_task_sender
        ...
        .await
    {
        error!("Failed to enqueue archive-only deletion task for task {task_id}: {e}");
        let problem = ProblemJson::from_status(
    ...
    info!("Queued archive-only deletion task for backup {task_id}");
